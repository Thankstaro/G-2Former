...
class Convolution(nn.Module):
    def __init__(self, input_size, out_size):
        ...
        self.weight = nn.Parameter(torch.FloatTensor(self.input_size, self.out_size))
    ...
    def forward(self, x, A, L):
        ...
        temp = temp - temp2
        emb = temp.clone()
        temp = torch.mm(temp, self.weight)
        return temp, emb

class Test(nn.Module):
    def __init__(self, num_layers, input_size, hidden_size, G_opt, num_o, unsup, device, gamma, randidx):
        super().__init__()
        ...
        self.num_o = num_o
        self.unsup = unsup
        self.indices = randidx
        self.relu = nn.ReLU()
        self.device = device
        self.is_act = True

        if num_layers == 1:
            setattr(self, 'gnn_layer' + str(num_layers), Convolution(input_size, num_o))
        else:
            for i in range(1, num_layers):
                layer_size = hidden_size  if i != 1 else input_size
                setattr(self, 'gnn_layer' + str(i), Convolution(layer_size, hidden_size))
            setattr(self, 'gnn_layer' + str(num_layers), Convolution(hidden_size, num_o))

        if self.is_act:
            self.weight3 = nn.Parameter(torch.FloatTensor(self.hidden_size, self.input_size))
            self.weight4 = nn.Parameter(torch.FloatTensor(self.input_size, self.hidden_size))
    ...
    def forward(self, x, is_act=True):
        ...
        pre_emb = x.clone()
        if self.is_act:
            A_u = (self.G_opt[0][3]).to(self.device)
            A_l = (self.G_opt[0][4]).to(self.device)
            
            pre_emb[self.indices] = pre_emb.clone()
            pre_emb = torch.spmm(A_u, pre_emb)
            pre_emb = torch.mm(pre_emb, self.weight4)
            pre_emb = self.relu(pre_emb)
            pre_emb = torch.spmm(A_l, pre_emb)
            pre_emb = torch.mm(pre_emb, self.weight3)
            pre_emb = self.relu(pre_emb)
            pre_emb = pre_emb.clone()[self.indices]
            pre_emb = pre_emb + x
        final_emb = pre_emb.clone()
        self.G_opt[0][2] = self.G_opt[0][2].to(self.device)
        self.G_opt[0][5] = self.G_opt[0][5].to(self.device)

        for i in range(1, self.num_layers + 1):
            temp_gnn = getattr(self, 'gnn_layer' + str(i))
            if i != self.num_layers:
                pre_emb, _ = temp_gnn(pre_emb, self.G_opt[0][5], self.G_opt[0][2])
                pre_emb = self.relu(pre_emb)
            else:
                pred, _ = temp_gnn(pre_emb, self.G_opt[0][5], self.G_opt[0][2])
        # pre_emb = torch.cat((scale1, (scale2+scale1)/2, (scale1+scale3)/2, (scale1+scale4)/2), 1)
        # pre_emb = (final_emb - final_emb.mean(0)) / (final_emb.std(0)+1e-10)

        return (torch.sigmoid(pred)).squeeze(1), [final_emb], vis
                

class Test2(nn.Module):
    def __init__(self, input_size, hidden_size, G_opt, num_o, mode, device):
        super().__init__()
        ...
        if mode == 'mlp':
            ...
            self.linear2 = nn.Linear(hidden_size, num_o)
        else:
            self.weight = nn.Parameter(torch.FloatTensor(self.input_size, self.hidden_size))
            self.weight2 = nn.Parameter(torch.FloatTensor(self.hidden_size, self.num_o))
            self.weight3 = nn.Parameter(torch.FloatTensor(self.hidden_size, self.input_size))
            self.weight4 = nn.Parameter(torch.FloatTensor(self.input_size, self.hidden_size))
    ...
    def forward(self, x):
        ...
        pre_emb = x
        if self.is_act:
            A_u = (self.G_opt[3]).to(self.device)
            A_l = (self.G_opt[4]).to(self.device)
        ...
        if self.mode == 'mlp':
            pre_emb = self.linear1(pre_emb)
            pre_emb = self.relu(pre_emb)
            pre_emb = self.linear2(pre_emb)
        else:
            self.G = self.G_opt[5].to(self.device)
            pre_emb = torch.spmm(self.G, pre_emb) - pre_emb
            pre_emb = torch.mm(pre_emb, self.weight)
            pre_emb = self.relu(pre_emb)
            pre_emb = torch.spmm(self.G, pre_emb) - pre_emb
            pre_emb = torch.mm(pre_emb, self.weight2)

        return (torch.sigmoid(pre_emb)).squeeze(1), [pre_emb], vis
